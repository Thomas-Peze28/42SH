...
static int handle_env_navigation_commands(char **warray,
    char ***env, history_t *history, alias_t *aliases)
{
    char *oldpwd = getcwd(NULL, 0);

    if (handle_history_command(warray, history))
        return 0;
    if (my_strcmp(warray[0], "alias"))
        return handle_alias_command(warray, aliases);
    if (my_strcmp(warray[0], "unalias"))
        return handle_unalias_command(warray, aliases);
...
static int handle_utility_commands(char **warray, char ***env)
{
    ...
    char ***env, history_t *history, alias_t *aliases)
{
    ...
    ret = handle_env_navigation_commands(warray, env, history, aliases);
    if (ret != -1)
        return ret;
    ret = handle_utility_commands(warray, env);
    if (ret != -1)
        return ret;
    return -1;
}

int gest_comm(char **warray, char ***env, history_t *history, alias_t *aliases)
{
    int ret;
    ...
    ret = handle_builtin_commands(warray, env, history, aliases);
    if (ret != -1)
        return ret;
